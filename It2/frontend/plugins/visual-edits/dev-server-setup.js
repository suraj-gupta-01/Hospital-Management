...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow additional trusted domains via env (comma-separated), default none
      const extraAllowed = process.env.ALLOWED_ORIGINS || "";
      if (extraAllowed) {
        const parts = extraAllowed.split(",").map((s) => s.trim()).filter(Boolean);
        for (const p of parts) {
          try {
            const re = new RegExp(p);
            if (origin.match(re)) return true;
          } catch (e) {
